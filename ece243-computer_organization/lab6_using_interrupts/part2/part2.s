/* 	
	Main program that counts on LEDs with toggle mode by pushbuttons
*/
...
KEY_ISR:	PUSH	{R0-R12}
			LDR		R3, =KEY_ADDR
			LDR		R4, [R3, #0xC]			// read edge capture register
			STR		R4, [R3, #0xC]			// clear the interrupt
			LDR		R5, RUN					// RUN value
			CMP		R5, #0
			MOVEQ	R6, #1
			STREQ	R6, RUN
			BEQ		key_isr_end
