function [ fourierSeriesCoefficents ] = DiscreteTimeFourierSeries2D( timeMatrix )
    %preallocate the transform matrix
    [M,N] = size(timeMatrix);
    fourierSeriesCoefficents = zeros(M,N);
    ...
    for l = 0 : M-1 %loop through the row requencies
        for k = 0 : N-1 %loop through the column frequencies
            ...
            % 1) Flatten both matrices
            timeMatrix = timeMatrix(:);
            fourierBasisVector2D_flatten = FourierBasisVector2D(l,k,M,N);
            fourierBasisVector2D_flatten = fourierBasisVector2D_flatten(:);
            % 2) Vectorization
            fourierSeriesCoefficents(l+1,k+1) = timeMatrix' * conj(fourierBasisVector2D_flatten);
        end
    end
end
